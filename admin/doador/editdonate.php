...
if(isset($_COOKIE['logadoadmin']) && $_COOKIE['logadoadmin'] == 'sim'){
    ...
    if(!$doador->existsDonate($id_doacao)){
        header("Location: index.php?p=home");
    }

    $return = $doador->getDonate($id_doacao);
...
<form action="<?php $_SERVER['PHP_SELF'];?>" method="post" name="frmeditar" class="border" enctype="multipart/form-data">
    <div class="p-5">
        <div class="form-group">
            <p class="fonte-impact text-center texto-amarelo font-weight-bold display-3">EDITAR DOAÇÃO</p>
        </div>
        <div class="form-group">
...
<?php
    ...
    if(isset($_POST['salvar'])) {
        ...
        if($doador->editDonate($id_doacao, $tipodoacao, $detalhes)){
            ?>
            <div class="alert alert-success mt-3" role="alert">
                Você editou a doação com sucesso.
            </div>
            <?php
    ...
    if(isset($_POST['excluir'])) {
        ...
        if($doador->deleteDonate($id_doacao)){
            ?>
            <div class="alert alert-success mt-3" role="alert">
