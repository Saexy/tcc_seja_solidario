...
<form action="<?php $_SERVER['PHP_SELF'];?>" method="post" name="frmedit" class="border" enctype="multipart/form-data">
    <div class="p-5">
        ...
        <div class="form-group">
            ...
            <?php echo '<input type="text" class="form-control" name="endereco" id="endereco" value="'.$doador->endereco.'" required>'; ?>
        </div>
        <div class="form-group">
            <label for="numerocasa">Número da casa:</label>
            <?php echo '<input type="text" class="form-control" name="numerocasa" id="numerocasa" value="'.$doador->numerocasa.'" required>'; ?>
        </div>
        <div class="form-group">
            <label for="sexo">Sexo:</label>
            <select class="form-control" name="sexo" id="sexo">
                <?php echo "<option selected hidden>$doador->sexo</option>"; ?>
...
if(isset($_POST['salvar'])) {
    ...
    if($doador->edit($endereco, $numerocasa, $sexo)){
        ?>
        <div class="alert alert-success mt-3" role="alert">
            Você editou os dados com sucesso.
        </div>
        <?php
        echo '<meta http-equiv="refresh" CONTENT="2;URL=?p=doador/profile&id='.$doador->id.'">';
    }else{
        ?>
...
<form action="<?php $_SERVER['PHP_SELF'];?>" method="post" name="frmadicionar" class="border" enctype="multipart/form-data">
    <div class="p-5">
        <div class="form-group">
            <p class="fonte-impact text-center texto-amarelo font-weight-bold display-3">ADICIONAR DOAÇÃO</p>
        </div>
        <div class="form-group">
...
if(isset($_POST['adicionar'])) {
    ...
    if($doador->addDonate($tipodoacao, $detalhes)){
        ?>
        <div class="alert alert-success mt-3" role="alert">
            Você adicionou uma doação com sucesso.
        </div>
        <?php
        echo '<meta http-equiv="refresh" CONTENT="2;URL=?p=doador/profile&id='.$doador->id.'">';
    }else{
    ...
    <div class="col-md-12 p-5">
        ...
        if($doador->historyDonate()){
            ...
            foreach($list as $history){
                ...
                <div class="row border mt-4">
                    ...
                    <div class="col-md-2 ml-5 d-flex justify-content-end align-items-center mt-2 mb-2">
                        <?php echo '<a class="text-center texto-azul" href="?p=doador/editdonate&id='.$history[1].'&id_doacao='.$history[0].'"><i class="material-icons align-middle">edit</i> Editar</a>'; ?>
                    </div>
                </div>
            ...
            <div class="border p-5">
                <p class="text-center texto-vermelho" style="font-size: 22.5px;">Este usuário não fez nenhuma doação.</p>
            </div>
            <?php
        }
        ?>
